...
import useWalletBalance from "../hooks/useWalletBalance"; // ✅ adjust path if needed
...
function Lottery() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      fetch(`http://localhost:1000/api/users/time`)
        .then((res) => res.json())
        .then((data) => {
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      const res = await fetch("http://localhost:1000/api/users/bet", {
        method: "POST",
        headers: {
  ...
  const sendEmptyBetToBackend = async () => {
    ...
    try {
      const res = await fetch("http://localhost:1000/api/users/play", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const resetGameStateBackend = async () => {
    try {
      const res = await fetch(`http://localhost:1000/api/users/reset`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const fetchGameHistory = async (page = 1) => {
    try {
      const res = await fetch(
        `http://localhost:1000/api/users/history?page=${page}`
      );
  ...
  return (
    <>
      ...
      <div className="px-4 mb-3">
        <div className="flex justify-between items-center bg-gradient-to-r from-yellow-600 to-yellow-500 rounded-xl shadow-lg px-4 py-2 text-black">
...
{/* Result Messages (Centered Sticky Top Message) */}
<div className="fixed inset-0 z-50 flex flex-col items-center justify-center space-y-3 pointer-events-none">
  {bigSmallResultMessage && (
    <div
      className={`min-w-[250px] px-6 py-3 text-center rounded-xl font-semibold text-lg shadow-xl transition-all duration-300 ${
    ...
    <div
      className={`min-w-[250px] px-6 py-3 text-center rounded-xl font-semibold text-lg shadow-xl transition-all duration-300 ${
        numberResultMessage.includes("Win")
          ? "bg-green-600 text-blue-200"
          : "bg-red-600 text-blue-200"
      }`}
    >
      {numberResultMessage}
    </div>
    ...
    >
      ...
      <div className="flex gap-2 justify-center mb-4">
        ...
        disabled={!isSelectionAllowed}
          className={`bg-green-400  font-semibold px-6 py-2 rounded shadow ${
            selectedColor === "Green" ? "ring-gray-900 bg-yellow-300 text-black " : "disabled:opacity-50 disabled:cursor-not-allowed"
          }`}
          onClick={() => {
        ...
        <button
        disabled={!isSelectionAllowed}
          className={`bg-purple-600  font-semibold px-6 py-2 rounded shadow ${
            selectedColor === "Purple" ? "ring-4 ring-gray-900 bg-yellow-300 text-black" : "disabled:opacity-50 disabled:cursor-not-allowed"
          }`}
          onClick={() => {
            if (!isSelectionAllowed || selectedColorRef.current) return;
        ...
        <button
        disabled={!isSelectionAllowed}
          className={`bg-red-600  font-semibold px-6 py-2 rounded shadow ${
            selectedColor === "Red" ? "ring-gray-900 bg-yellow-300 text-black" : "disabled:opacity-50 disabled:cursor-not-allowed"
          }`}
          onClick={() => {
            if (!isSelectionAllowed || selectedColorRef.current) return;
  ...
  `}
      ...
      {/* User Selection */}
      <div className="bg-white/10 backdrop-blur-md rounded-2xl px-6 py-4 mx-4 mb-6 shadow-lg space-y-6 text-white">
        <div className="flex justify-center gap-4">
          <button
            disabled={!isSelectionAllowed}
            className={`w-32 py-2 rounded-l-full font-semibold transition  ${
              selectBigSmallRef.current === "Big"
                ? "ring-4 ring-gray-900 bg-yellow-300 text-black  shadow-white shadow-inner"
          ...
          <button
            ...
            className={`w-32 py-2 rounded-r-full font-semibold transition ${
              selectBigSmallRef.current === "Small"
                ? "ring-4 ring-gray-900 bg-yellow-300 text-black"
